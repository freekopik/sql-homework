...
INSERT INTO areas (title) SELECT 'City_' || i FROM generate_series(1, 100) s(i);
INSERT INTO industries (title) VALUES ('IT'), ('Маркетинг'), ('Продажи'), ('Медицина'), ('Финансы');

INSERT INTO specializations (title, industry_id) VALUES ('Backend', 1), ('Frontend', 1), ('Data Science', 1), ('SMM', 2), ('Sales', 3);
INSERT INTO specializations (title, industry_id) 
SELECT 'Spec_' || i, floor(random() * 5 + 1) FROM (SELECT i FROM generate_series(1, 500) i ORDER BY i OFFSET 0) s;

INSERT INTO skills (title) VALUES ('Python'), ('SQL'), ('PostgreSQL'), ('Docker'), ('Git'), ('React'), ('FastAPI'), ('Linux');
INSERT INTO skills (title) SELECT 'Skill_' || i FROM generate_series(1, 1000) s(i);

INSERT INTO employers (title, is_verified, rating, email, created_at)
SELECT 'Company ' || i, (random() > 0.4), (random() * 5), 'hr' || i || '@test.com', v_now 
...
SELECT 
    ...
    (SELECT id FROM employers OFFSET floor(random() * (SELECT count(*) FROM employers)) LIMIT 1),
    (SELECT id FROM vacancy_statuses OFFSET floor(random() * (SELECT count(*) FROM vacancy_statuses)) LIMIT 1),
    (SELECT id FROM specializations OFFSET floor(random() * (SELECT count(*) FROM specializations)) LIMIT 1),
    true,
    floor(random() * 3),
    floor(random() * 3 + 4),
    true,
...
SELECT 
    v.id,
    (SELECT id FROM areas OFFSET floor(random() * (SELECT count(*) FROM areas)) LIMIT 1)
FROM vacancies v;

INSERT INTO applicants (firstname, lastname, gender_id, email, created_at)
SELECT 'Name_' || i, 'Lastname_' || i, floor(random() * 4 + 1), 'user' || i || '@mail.ru', v_now 
FROM (SELECT i FROM generate_series(1, 150000) i ORDER BY i OFFSET 0) s;

INSERT INTO resumes (title, applicant_id, compensation_from, created_at)
SELECT 'Резюме ' || i, (floor(random() * 150000 + 1)), floor(random() * 16 + 4) * 10000, v_now - (random() * interval '365 days') 
...
SELECT 
    ...
    false 
FROM (
    SELECT r.id FROM (SELECT id FROM resumes ORDER BY id OFFSET 0) r CROSS JOIN generate_series(1, 2)
...
SELECT 
    ...
    true
FROM (
    SELECT id FROM resumes ORDER BY id OFFSET 0
) sub 
WHERE (random() > 0.3);

INSERT INTO resume_skills (resume_id, skill_id)
SELECT r.id, s.id FROM (SELECT id FROM resumes WHERE id % 10 = 0 ORDER BY id OFFSET 0) r 
CROSS JOIN LATERAL (SELECT id FROM (SELECT id FROM skills ORDER BY id OFFSET 0) sk ORDER BY random() LIMIT 5) s ON CONFLICT DO NOTHING;

INSERT INTO resume_views (resume_id, employer_id, viewed_at)
SELECT id, floor(random() * 1000 + 1), created_at + (random() * interval '30 days')
FROM (SELECT id, created_at FROM resumes WHERE id % 50 = 0 ORDER BY id OFFSET 0) r;

INSERT INTO vacancy_areas (vacancy_id, area_id)
SELECT id, floor(random() * 107 + 1) FROM (SELECT id FROM vacancies ORDER BY id OFFSET 0) v ON CONFLICT DO NOTHING;

INSERT INTO vacancy_skills (vacancy_id, skill_id)
SELECT v.id, s.id FROM (SELECT id FROM vacancies WHERE id % 2 = 0 ORDER BY id OFFSET 0) v 
CROSS JOIN LATERAL (SELECT id FROM (SELECT id FROM skills ORDER BY id OFFSET 0) sk ORDER BY random() LIMIT 10) s ON CONFLICT DO NOTHING;

INSERT INTO responses (applicant_id, vacancy_id, resume_id, response_status_id, created_at)
SELECT 
    r.applicant_id, 
    v.id, 
    r.id, 
...
FROM (
    SELECT id, created_at FROM vacancies ORDER BY id OFFSET 0
) v 
CROSS JOIN LATERAL (
